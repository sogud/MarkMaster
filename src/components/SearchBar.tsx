...
const SearchBar: React.FC<SearchBarProps> = ({ currentFolder, onSearchResult }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [isSearching, setIsSearching] = useState(false);

  const searchBookmarks = (query: string, folder: BookmarkFolder): (Bookmark | BookmarkFolder)[] => {
    ...
    for (const item of folder.children) {
      ...
      if (!('url' in item) && item.children) {
        results = [...results, ...searchBookmarks(query, item as BookmarkFolder)];
      }
    }
